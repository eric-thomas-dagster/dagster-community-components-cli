...
| **[One-Component RAG, Driven by Dynamic Queries](rag_pipeline_dynamic.md)** | `rag_pipeline` (embed + retrieve + generate in one asset) partitioned by `query_id` — one materialization per query, addressable via `dg launch --partition <id>`. | OpenAI-compatible for the LLM step | You don't need to swap individual pipeline steps and just want per-query addressability + retry + run history without adding queue infrastructure. |
