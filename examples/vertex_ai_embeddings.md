...
- `embeddings_generator` — generic LiteLLM-routed embeddings.
- `pgvector_asset` / `pinecone_asset` / `chromadb_asset` — load embeddings into a vector store downstream.
- `vector_store_query` — query a vector store with the embeddings.
- `gemini_llm` / `openai_llm` / `anthropic_llm` — generate text on the same SA / pattern.
