...
| [Agentic Router — LLM Picks the Next Action, Human Gates the Exit](agentic_router.md) | `dataframe_from_csv`, `iterative_supervisor_agent` (6 bounded tools), `llm_prompt_executor`, `human_approval_gate`, `dataframe_to_csv` (×2), `duckdb_table_writer`, `filesystem_monitor`, `asset_job` | **OpenAI-compatible key** | Router-loop shape from the BPMN "agentic orchestration" pitch. Baggage-loss triage: planner loops over a bounded tool set (query_db / inquire_airport / send_voucher / organize_delivery / inform_passenger / request_info), each step is its own asset, classifier + auto-approver + human-review gate + compensation branch + legacy audit. c1+c3 auto-cascade, c2 pends for human. Live-validated end-to-end. |
