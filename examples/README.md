...
| **[Deploying to Dagster+ (umbrella)](deploying.md)** | The one-stop concept doc — code location vs deployment terminology, agents + queues, branch deployments, Serverless vs Hybrid, dependency handling, plus a scenario matrix pointing at the right tool for every shape. Analog to Prefect's `/concepts/deployments`. Start here if you're new to Dagster+ deployment. |
| **[`dg-deploy` CLI wrapper — full reference](dg_deploy.md)** | Ergonomic single-command wrapper for `dg plus deploy`. Deploys a loose `.py`, folder of `.py`, existing dg-native project (including full `uvx create-dagster` projects), or auto-migrates a legacy `dagster_cloud.yaml`. Modes: Serverless (pex, no Docker), Hybrid (docker + push to your registry), `--dev` (local UI at :3000), `--run` (one-shot local materialize), `--from user/repo/path.py` (curl-and-deploy from GitHub), `--no-mutate` (CI-safe: refuses any project-dir writes). Source: [`lib/dg_deploy.sh`](./lib/dg_deploy.sh). |
| **[Single-file → Dagster+ Serverless (pex, no Docker)](single_file_serverless.md)** | The Prefect-parity story for Dagster+ Serverless. 2 files, ~30 lines of boilerplate around your actual code. Deploys as a **pex bundle** — no Docker, no image registry, no build step. Projects: [`serverless_minimal/`](./serverless_minimal/) (floor — 2 assets, zero external services), [`data_engineering_serverless/`](./data_engineering_serverless/) (real content — 5-asset HN pipeline, no API keys), [`agentic_tour_serverless/`](./agentic_tour_serverless/) (richer LLM example — needs `OPENAI_API_KEY`). Wrapper CLI at [`lib/dg_deploy.sh`](./lib/dg_deploy.sh) — `bash dg-deploy my_flow.py` auto-detects deps + `--from user/repo/path.py` GitHub fetch. **Locations verified live at a private Dagster+ prod deployment**: `hello`, `cli-verify`, `data-engineering`, `migrated-verify`, `migrated-hybrid-verify`, `inplace-verify`, `fullproj-verify`. |
| **[Hybrid git-runner — deploy once, iterate with `git push`](hybrid_git_runner/README.md)** | The Hybrid answer. A runner container (subclass of [`script_orchestrator`](https://github.com/eric-thomas-dagster/script_scheduling_and_orchestration)'s `ScriptGithubComponent`) deployed ONCE to your Hybrid agent; new flows appear in Dagster+ by pushing them to a git repo it watches. Docker enters the picture ONLY for this one-time image build; iteration is `git push`. Three deploy modes (prebuilt / build+push / --image override). Multi-deployment topology (prod/staging/dev with same runner + different env vars). Branch deployments auto-route to PR branches. |
| **[Prefect vs. Dagster+ — the single-file story](prefect_vs_dagster_single_file.md)** | Honest 3-way side-by-side (Prefect Managed / Dagster+ Serverless via pex / Dagster+ Hybrid via git-runner). Both Dagster+ paths skip the "docker build per iteration" cost that would otherwise show up in a naive Hybrid deploy. Multi-deployment + branch-deployment differentiators Prefect doesn't cleanly match. |
| **[Event Automation (Prefect Automations shape)](event_automation.md)** | Prefect's Automations page as ONE Dagster YAML component. **35 trigger types** covering every observable state in Dagster: run lifecycle (`run_status` / `run_duration` / `run_stuck` / `run_startup_slow`), asset events (`asset_materialized` / `asset_observation` / `asset_check_failed`), data quality (`metric_threshold` / `metadata_match` / `asset_value_change` / `freshness_violation` / `absence`), platform health (`daemon_heartbeat` / `code_location_status` / `sensor_failing` / `concurrency_hit`), errors + logs (`step_error` / `log_pattern` with stdout+stderr for K8s/ECS container output), external (`schedule` / `http_poll` / `sqs_poll`), plus **17 action types** (`materialize` / `launch_job` / `cancel_run` / `retry_run` / `webhook` / `slack` / `pagerduty` / `opsgenie` / `discord` / `teams` / `mattermost` / `email` / `sns` / `sqs` / `toggle_sensor` / `toggle_schedule` / `emit_event`). Compound `all_of` + `any_of` triggers give AND/OR logic. Under the hood every automation becomes a real Dagster sensor — full UI, tick history, no separate mini-app. Comprehensive pytest suite (56 tests, all passing). `bash setup_event_automation_demo.sh` for a live demo that fires a webhook on run failure + cron heartbeat. |
...
| **[Maintainer Investigation Room — the "AI agentic-orchestration" pattern, Dagster-honest](maintainer_investigation_room.md)** | AI-driven GitHub issue triage as a real Dagster asset graph. Single-YAML `AgenticPipelineComponent` v2: GitHub MCP fetch (via the new `mcp_call` op) → 4 typed-input specialists → 6-input `preliminary` synthesize → skeptic → 4-input `decision` synthesize → maintainer-facing report, gated on a human-signed JSON token. Every port is an asset key — downstream dbt / warehouse assets depend on them, Insights alerts key off the approval check. Ships with a zero-to-something test: a fresh Claude Code session with only `CLAUDE.md` installed composed a validating defs.yaml from an NL prompt in one pass. | ~$0.03 |
| **[Maintainer Investigation Room v3 — every new AI-agentic primitive, one pipeline](maintainer_investigation_room_v3.md)** | The **comprehensive** MIR — same triage pattern rebuilt to exercise every AI-agentic primitive from v0.10.69–v0.10.73: `per_step_ops: true` (11 step_keys in the Runs page + native per-op retry), `fastmcp` transport, `tool_use_loop` (open-ended agent tool-use on repo exploration), `handoff` op to a LangGraph state machine (for the reproduction analysis), `debate` (3 skeptic proposers + arbitrator), `critique_loop` (drafter + critic × 2), plus `SlackApprovalGateComponent` for Slack quorum HITL. 11 first-class Dagster assets per triage, 20-30 total LLM calls per run. Dagster IS the harness at the pipeline level; LangGraph is one step; adjacent MCP + fan-out + HITL steps stay Dagster-native. | ~$0.05 |
| **[Typed named inputs — the "any op joins from any prior op by port name" primitive](typed_named_inputs.md)** | The v2 wiring shape that makes the maintainer-investigation demo work. Adds `inputs: {port_name: {from: step_id} \| {literal: value}}` to every text-emitting op in `AgenticPipelineComponent`. Each port becomes a `{port_name}` placeholder in `prompt_template` + `system_prompt` (and, for `mcp_call`, in string `tool_args`). Standard typed-I/O execution-plan shape, on Dagster's substrate where every port becomes a queryable asset key. Backward-compat: `source: <id>` and `sources: [<ids>]` still work. | — |
...
| [**Fortune 500 Data Platform POC**](f500_poc.md) | `DatabaseQueryComponent`, `DataVaultHubLinkSatelliteComponent`, `TrinoQueryComponent`, `MinIOIOManagerComponent`, `DuckDBTableWriterComponent`, `FreshnessPolicyComponent`, `ShellCommandAssetComponent`, `K8sJobAssetComponent`, `OtlpComputeLogManager` | **Big one** — 6 containers (Postgres + MinIO + Trino + Elasticsearch + Kibana + OpenTelemetry Collector), 3 real Dagster code-locations (`sales/` / `marketing/` / `finance/`) under one `dg.toml` workspace with cross-code-location asset deps. Data Vault 2.0 hub/link/sat in sales, cross-loc dep from marketing/campaign_attribution → sales/dv2/customer_hub, Trino federated read joining postgres.finance + postgres.sales in finance, `FreshnessPolicyComponent` YAML for cross-domain quality gate (100% components — no custom Python), `OtlpComputeLogManager` ships every op's stdout/stderr through OTel Collector into an Elasticsearch data stream browsable in Kibana. Doc includes a "Swapping to GCP" component-substitution guide. |
| [**Retail Data Orchestration (3-scenario)**](retail_data_orchestration.md) — companion docs: [REAL POC from-scratch](retail_data_orchestration_real_from_scratch.md) · [local from-scratch](retail_data_orchestration_from_scratch.md) · [real-mode swap guide](retail_data_orchestration_real_mode.md) | `ShellCommandAssetComponent`, `FilesystemMonitorSensorComponent`, `FreshnessCheckComponent`, `EventAutomationComponent` + `dagster-powerbi` (official) + real-mode swaps to `SnowflakeSnowpipeLoadSensorComponent` / `DbtRunJobComponent` / `DbtCloudJobSensorComponent` / `HvrHubWorkspaceComponent` / `DbtStateReusePatchComponent` | Enterprise orchestrator POC shape — three canonical scenarios in one project: (1) Python extract → object-store landing → load-completion-sensor trigger → dbt build (trigger on load-completion EVIDENCE, not a timer); (2) external replication-API refresh → dbt build; (3) state-awareness skip pattern (defer-to-dbt-state AND orchestrator-native FreshnessPolicy). Includes a companion [real-mode swap guide](retail_data_orchestration_real_mode.md) (single-line YAML replacements from local stand-ins → real Snowflake / dbt Cloud / HVR / Power BI). Bundled criteria mapping table maps every OBS-XX / TRG-XX / REC-XX / INT-XX / ERR-XX / ALT-XX / DEV-XX / OPS-XX evaluation row to the specific component that satisfies it. Components install as **files inside the project** (`dagster-component add`), not as a library import — customers can `cat` every component's source. |
...
| [Elasticsearch Asset (Docker ES)](elasticsearch_asset.md) | `synthetic_data_generator`, `elasticsearch_asset` | `mode: index` — upstream DataFrame → bulk-index into a Docker ES single-node. 100 fake products indexed with `product_id` as `_id`. Sibling of the existing `elasticsearch.md` demo (which uses `elasticsearch_reader` for the ES → DataFrame direction). |
| [pgvector Reader (Docker Postgres+pgvector)](pgvector_reader.md) | `pgvector_reader` | Hermetic — no external API keys required. Uses the `query_embedding` literal path (bypasses the OpenAI `query_text` path). Docker `pgvector/pgvector:pg16`, 8 hand-tuned 4-dim vectors + IVFFLAT cosine index, deterministic top-5 ranking (`ml basics` closest, `databases 101` farthest). Retargets at any Postgres+pgvector instance (RDS / Cloud SQL / Neon / Supabase / self-hosted). |
| [pgvector Asset (Ollama + Docker Postgres, both paths)](pgvector_asset.md) | `synthetic_data_generator`, `pgvector_asset` (×2), `litellm_embedding_batch` | Hermetic — Ollama runs the embedder LOCALLY (`ollama/nomic-embed-text`). Exercises both embedding paths of the refactored pgvector_asset (v1.2.0): **inline** (embeds via LiteLLM+Ollama in one step) and **precomputed** (composes `litellm_embedding_batch` upstream, pgvector_asset skips the embedder). Verifies both target tables land 100/100 byte-for-byte identical vectors. Swap `embedding_model` to any LiteLLM-supported provider (OpenAI, Voyage, Cohere, HF TEI, Azure) with no other config change. |
...
| [Control-M Integration](controlm_integration.md) | `controlm_integration` | Wraps each Control-M job as a daily-partitioned Dagster asset (Automation API v2). Ships 5 op-backed jobs (restart / hold / free / kill / reconcile) + 2 sensors (external-execution monitor + inbound trigger) + hourly reconciliation schedule. `demo_mode: true` simulates the full API lifecycle on stdout — zero external deps. Flip to `false` + set `CONTROLM_USER`/`CONTROLM_PASSWORD` for real Control-M. Hybrid-deployment shape (not a Control-M killer — pairs with it). |
| [RunMyJobs Integration](runmyjobs_integration.md) | `runmyjobs_integration` | Wraps each Redwood RunMyJobs (SAP Redwood Scheduler) JobDefinition as a daily-partitioned Dagster asset (REST API, HTTP Basic). Same asset/op/sensor/schedule surface as `controlm_integration` — shops running both can present one Dagster pane of glass. `demo_mode: true` simulates the full REST lifecycle on stdout — zero external deps. Flip to `false` + set `RUNMYJOBS_USER`/`RUNMYJOBS_PASSWORD` for real RMJ. |
| [Jenkins Integration](jenkins_integration.md) | `jenkins_integration` | Wraps each Jenkins job as a daily-partitioned Dagster asset (REST + CSRF crumb + API token). 5 op-backed jobs (rebuild / disable / enable / stop / reconcile), external-execution monitor + inbound-trigger sensors, hourly reconciliation. `demo_mode: true` simulates end-to-end on stdout; flip false + set `JENKINS_USER`/`JENKINS_API_TOKEN` for a real controller. `jenkins/jenkins:lts` public Docker image works out of the box for real-mode testing. Framed as the transition-phase shape — Dagster owns new pipelines, Jenkins keeps what it already runs. |
| [Rundeck Integration](rundeck_integration.md) | `rundeck_integration` | Wraps each Rundeck job as a daily-partitioned Dagster asset (REST + `X-Rundeck-Auth-Token`). 5 op-backed jobs (restart / disable / enable / abort / reconcile), 2 sensors, hourly reconciliation. `demo_mode: true` simulates end-to-end on stdout; flip false + set `RUNDECK_API_TOKEN` for a real Rundeck instance. `rundeck/rundeck` open-source Docker image works out of the box for real-mode testing. |
| [Stonebranch UAC Integration](stonebranch_uac_integration.md) | `stonebranch_uac_integration` | Wraps each Stonebranch Universal Automation Center (UAC) task as a daily-partitioned Dagster asset (REST + HTTP Basic). 5 op-backed jobs (rerun / hold / release / cancel / reconcile), 2 sensors, hourly reconciliation. `demo_mode: true` simulates end-to-end on stdout; flip false + set `STONEBRANCH_USER`/`STONEBRANCH_PASSWORD` for a real UAC server. `stonebranch/uac-demo` Docker image available (trial account required). |
| [IBM Workload Scheduler Integration](iws_integration.md) | `iws_integration` | Wraps each IBM Workload Scheduler (IWS/TWS, formerly Tivoli Workload Scheduler) job as a daily-partitioned Dagster asset (REST + HTTP Basic). 5 op-backed jobs (rerun / hold / release / kill / reconcile), 2 sensors, hourly reconciliation. Distributed engine uses `/twsd/v1`; z/OS uses `/twsz/v1`. `demo_mode: true` simulates end-to-end on stdout; flip false + set `IWS_USER`/`IWS_PASSWORD` for a real IWS controller. IBM `icr.io/wa-container/*` images available (BYOL license-gated). |
| [ActiveBatch Integration](activebatch_integration.md) | `activebatch_integration` | Wraps each ActiveBatch (by Redwood) job as a daily-partitioned Dagster asset (REST + HTTP Basic). 5 op-backed jobs (restart / hold / release / abort / reconcile), 2 sensors, hourly reconciliation. Windows-native; no public Docker image — `demo_mode: true` simulates end-to-end on stdout; flip false + set `ACTIVEBATCH_USER`/`ACTIVEBATCH_PASSWORD` for a real ActiveBatch controller. Sibling to `runmyjobs_integration` (both Redwood). |
| [JAMS Scheduler Integration](jams_integration.md) | `jams_integration` | Wraps each JAMS Scheduler (Fortra/HelpSystems) job as a daily-partitioned Dagster asset (REST + HTTP Basic). 5 op-backed jobs (restart / hold / release / cancel / reconcile), 2 sensors, hourly reconciliation. Windows-native; no public Docker image — `demo_mode: true` simulates end-to-end on stdout; flip false + set `JAMS_USER`/`JAMS_PASSWORD` for a real JAMS server. |
| [UiPath Orchestrator Integration](uipath_orchestrator_integration.md) | `uipath_orchestrator_integration` | Wraps each UiPath process (release) as a daily-partitioned Dagster asset (REST + OAuth2 client credentials). 5 op-backed jobs (restart / soft-stop / kill / disable-schedule / reconcile), 2 sensors, hourly reconciliation. `demo_mode: true` simulates end-to-end on stdout; flip false + set `UIPATH_CLIENT_ID`/`UIPATH_CLIENT_SECRET` for a real Orchestrator. UiPath Cloud Community tier is free at cloud.uipath.com/signup for real-mode testing. |
| [Automation Anywhere Integration](automation_anywhere_integration.md) | `automation_anywhere_integration` | Wraps each A360 bot deployment as a daily-partitioned Dagster asset (REST + JWT). 5 op-backed jobs (redeploy / pause / resume / stop / reconcile), 2 sensors, hourly reconciliation. `demo_mode: true` simulates end-to-end on stdout; flip false + set `AUTOMATION_ANYWHERE_USER`/`AUTOMATION_ANYWHERE_PASSWORD` for a real Control Room. AAI Cloud tier available with a business trial account. |
| [Blue Prism Integration](blue_prism_integration.md) | `blue_prism_integration` | Wraps each Blue Prism process session as a daily-partitioned Dagster asset (REST + Basic against Blue Prism 7+ Web API). 5 op-backed jobs (restart / stop / terminate / hold / reconcile), 2 sensors, hourly reconciliation. `demo_mode: true` simulates end-to-end on stdout; flip false + set `BLUE_PRISM_USER`/`BLUE_PRISM_PASSWORD` for a real controller. Windows-native; no public Docker. |
| [Microsoft Power Automate Integration](power_automate_integration.md) | `power_automate_integration` | Wraps each Power Automate cloud flow as a daily-partitioned Dagster asset (REST + OAuth2 via Azure AD/Entra). 5 op-backed jobs (retrigger / cancel / turn-off / turn-on / reconcile), 2 sensors, hourly reconciliation. `demo_mode: true` simulates end-to-end on stdout; flip false + set `POWER_AUTOMATE_TENANT_ID`/`POWER_AUTOMATE_CLIENT_ID`/`POWER_AUTOMATE_CLIENT_SECRET` for a real environment. Cloud-only Microsoft SaaS — no Docker. |
| [Event Automation (Prefect Automations shape)](event_automation.md) | `event_automation` | Prefect Automations UX as one YAML component — 35 triggers → 17 actions in a single declarative block. Full noise-reduction suite (11 knobs): rate limits, business-hours gating, maintenance windows, root-cause correlation suppression, escalation ladders, auto-resolve, LLM-decides-whether-to-page, retry-aware step failures. Asset-selection strings on `asset_keys` (`group:X`, `tag:foo=bar`, `kind:Y`, `marts/*`). Run-filter mixin (`job_name_pattern`, `run_tags`). Cross-code-location targeting via `monitored_locations` — deploy one dedicated alerts location that watches every prod location. Multi-dim partition support (`partition_key: {date: '{partition_date}', region: '{partition_region}'}`). |
...
| [Dagster+ MCP (materialize Dagster+ ops as assets)](mcp_dagster_plus.md) | `MCPToolCallComponent` | Wire Dagster+ MCP (`https://mcp.agent.dagster.cloud/mcp/`) via HTTP transport + two headers (`Authorization: Bearer <token>` + `Dagster-Cloud-Organization`). Every tool your org's MCP exposes — recent runs, asset lineage, deployment health — becomes a first-class Dagster asset with materialization history + freshness. Includes a `tools/list` discovery snippet. |
| [Stripe MCP (materialize Stripe ops as assets)](mcp_stripe.md) | `MCPToolCallComponent` | Wire Stripe's official MCP server (`https://mcp.stripe.com/`) via HTTP transport + one header (`Authorization: Bearer <restricted-key>`). Every Stripe MCP tool — customers, charges, balances, payment links — becomes a first-class Dagster asset. Complements the dedicated `StripeIngestion` + `StripeWorkspace` components (which use the REST SDK). |
| [MariaDB (via MySQL components)](mariadb.md) | `mysql_resource`, `synthetic_data_generator`, `dataframe_to_table` | Docker-local `mariadb:11` validated against the existing MySQL components — MariaDB is wire-compatible so the same YAML that works against MySQL 8 works here unchanged. 100 synthetic rows → SQLAlchemy sink → `analytics.orders_snapshot`. Setup script sidesteps MariaDB 11's default TLS + unix_socket quirks. |
| [Memcached (resource + flush job)](memcached.md) | `memcached_resource`, `memcached_cache_flush_job` | Two brand-new components validated against `memcached:1.6-alpine`. Setup script seeds 3 keys via the text protocol, runs the flush job through Dagster, then verifies the keys are gone. Memcached has no SCAN so pattern-match deletes aren't supported; use Redis for glob-based invalidation. |
...
| [Cache expensive Python compute — skip re-runs when nothing changed](cached_asset.md) | `cached_asset` (`@cached` decorator on a `@dg.asset`) | 100% offline poster demo for the DCC decorator family. Three back-to-back runs of one asset: RUN 1 MISS (writes parquet cache), RUN 2 HIT (loads parquet, skips compute), RUN 3 MISS after `code_version` bump (new key, new parquet). Ends with an event-log query reading cache metadata (`cache_status` / `cache_key` / `cache_path` / `cache_rows`) — the shape you'd use for a cache-hit-rate dashboard. |
| [Track $ per asset — rolling-window cap + pre-flight breach guard](budget_asset.md) | `budget_asset`, `synthetic_data_generator` | 100% offline. Demonstrates BOTH shapes: (1) `@budget` Python decorator over `@dg.asset` with a user `cost_fn` — 3 runs of $0.40 each; cumulative crosses $1.00 cap on RUN 3 → `budget_breach=true`, (2) `BudgetAssetComponent { wraps: SyntheticDataGeneratorComponent }` — the "money shot" composability with `cost_per_second: 100.0` blowing through the cap on the very first run. Both share event-log-backed cost history + breach state — the shape you'd use for a FinOps dashboard. |
| [Wall-clock SLAs on asset compute — with escalation-after-N-breaches](sla_asset.md) | `sla_asset`, `synthetic_data_generator` | 100% offline. Demonstrates BOTH shapes: (1) `@sla` Python decorator over `@dg.asset` — 4 runs controlled by `SLEEP_SECONDS`; 3 back-to-back breaches → 3rd tagged `sla_escalated=true` (sensor-actionable), (2) `SlaAssetComponent { wraps: SyntheticDataGeneratorComponent }` — the "money shot" composability where an outer decorator component stacks over an inner data-shape component with zero Python. Both share the same breach observation format. Different from `freshness_policy` (source freshness) — this measures compute duration. |
| [Classification-aware retry — transient vs permanent, no thrashing](smart_retry.md) | `smart_retry`, `synthetic_data_generator` | 100% offline. Demonstrates BOTH shapes: (1) `@smart_retry` Python decorator over `@dg.asset` — 3 modes (`success`/`flaky`/`permanent`) exercise transient-retries-via-`STEP_RESTARTED` and permanent-`dg.Failure` paths, (2) `SmartRetryComponent { wraps: SyntheticDataGeneratorComponent }` — the "money shot" composability where classification-aware retry layers over another component's compute with zero Python. Both share the same classification engine. Complement to Dagster's built-in `RetryPolicy` (which retries every failure the same way). |
| [Rate-limit an asset — cross-run mutex backed by the event log](throttle_asset.md) | `throttle_asset`, `synthetic_data_generator` | 100% offline. Demonstrates BOTH shapes: (1) `@throttle` Python decorator over `@dg.asset`, (2) `ThrottleAssetComponent { wraps: SyntheticDataGeneratorComponent }` — the "money shot" composability where an outer decorator component stacks over an inner data-shape component with zero Python. Both share the same event-log rate-limit state and emit the same `throttle_skipped` observations. |
| [Hard-kill runaway compute at N seconds](timeout_asset.md) | `timeout_asset`, `synthetic_data_generator` | 100% offline. Demonstrates BOTH shapes: (1) `@timeout` Python decorator hard-kills a `@dg.asset` at N seconds (`SLEEP_SECONDS=0.3` → OK; `SLEEP_SECONDS=2.0` → `dg.Failure`), (2) `TimeoutAssetComponent { wraps: SyntheticDataGeneratorComponent }` — the "money shot" composability wraps an inner 5000-row generator with a 10ms deadline. Both emit the same `timeout_hit` observations. Fills the real gap in Dagster's `RetryPolicy` (which has no timeout knob). Portable across every Dagster deployment shape via `concurrent.futures.ThreadPoolExecutor` (Serverless-safe — no `signal.SIGALRM` dependency). |
| [Dual-run new vs old — vendor swap without risking production](shadow_asset.md) | `shadow_asset`, `synthetic_data_generator` | 100% offline. Demonstrates BOTH shapes: (1) `@shadow` Python decorator dual-runs primary + shadow implementations on a `@dg.asset` (last row diverges → `shadow_match=false`, `diff_rows=1`), (2) `ShadowAssetComponent { wraps: PrimaryComp, shadow_wraps: AltComp }` — the "money shot" composability wraps TWO DCC components side-by-side (perfect for vendor swaps like `wraps: SnowflakeQueryComponent, shadow_wraps: BigqueryQueryComponent`). Both emit the same `shadow_match` observations. Migration playbook: ship shadow → verify convergence via the event log → flip primary → drop shadow. |
| [Point-in-time snapshots — with rollback + compression](snapshot_asset.md) | `snapshot_asset`, `synthetic_data_generator` | 100% offline. Demonstrates BOTH shapes: (1) `@snapshot` Python decorator writes a point-in-time parquet snapshot after every `@dg.asset` materialization, (2) `SnapshotAssetComponent { wraps: SyntheticDataGeneratorComponent }` — the "money shot" composability wraps an inner data-gen with snapshotting, zero Python. Path shape: `<uri>/<asset>/<code_version>/<timestamp>__<run_id>.<ext>`. Auto-detects format (DataFrame→parquet, dict→json, bytes→bin, str→text, else pickle). Every write leaves an `AssetObservation` with `snapshot_path` / `snapshot_bytes` / `snapshot_format` — rollback becomes an event log query. Works with any fsspec URI (local / `s3://` / `gs://` / `abfs://`). |
| [Scrub PII & secrets before they hit the event log](sensitive_asset.md) | `sensitive_asset`, `synthetic_data_generator` | 100% offline. Demonstrates BOTH shapes: (1) `@sensitive` Python decorator scrubs `ssn=... password=... api_token=...` inline in `context.log.*` calls + matching keys in `MaterializeResult.metadata` BEFORE they land in the event log, (2) `SensitiveAssetComponent { wraps: SyntheticDataGeneratorComponent }` — the "money shot" composability proxies an inner component's context.log through the redactor (`wraps: RestApiFetcherComponent` catches any secret an inner accidentally logs). Three strategies: `redact` → `[REDACTED]`, `hash` → `sha256:XXXXXXXX`, `mask` → `***last4`. Emits `sensitive_redacted_count` observation every run for SOC2 per-asset attestation. |
| [Auto-profile every materialization — histograms, quantiles, correlations](profile_asset.md) | `profile_asset`, `synthetic_data_generator` | 100% offline. Demonstrates BOTH shapes: (1) `@profile` Python decorator on a `@dg.asset` returning a 500-row DataFrame, (2) `ProfileAssetComponent { wraps: SyntheticDataGeneratorComponent }` — the outer profile extracts the DataFrame the inner returns and emits per-column `null_ratio` / `distinct_count` / `min` / `max` / `mean` / `std` as one `AssetObservation`. Every materialization is a JSON snapshot in the event log — drift detection = query the log, diff adjacent snapshots. |
| [Route Python `print()` into Dagster logs — legacy code gets observability](log_prints_asset.md) | `log_prints_asset`, `synthetic_data_generator` | 100% offline. Demonstrates BOTH shapes: (1) `@log_prints` Python decorator captures 4 `print()` calls into `context.log.info` with a `[legacy]` prefix, (2) `LogPrintsAssetComponent { wraps: SyntheticDataGeneratorComponent }` — outer redirects the inner's stdout to the run log; wrap is a transparent no-op when the inner is quiet. Best fit: porting legacy scripts that use `print()` for visibility without a rewrite. |
| [Safe-mode any asset — cost estimates + diff-vs-current, no writes](dry_run_asset.md) | `dry_run_asset`, `synthetic_data_generator` | 100% offline. Demonstrates BOTH shapes AND a semantic difference: (1) `@dry_run` Python decorator RUNS the compute + discards the return value, (2) `DryRunAssetComponent { enabled: true, wraps: SyntheticDataGeneratorComponent }` — inner compute is NEVER INVOKED (stronger dry-run, prevents inner's side effects too). Includes a control run with `enabled: false` proving passthrough. Enable via decorator arg, YAML field, run tag `dry_run=true`, or env `DAGSTER_DRY_RUN=1`. |
| [Prefect-style lifecycle callbacks — on_start / on_success / on_failure / on_end](hooks_asset.md) | `hooks_asset`, `synthetic_data_generator` | 100% offline. Demonstrates BOTH shapes AND both callback paths: (1a) `@on_hooks` Python decorator on a succeeding asset (multiple `on_success` hooks fire in list order), (1b) same on a failing asset (`on_failure` fires, then `RuntimeError` re-raised → STEP_FAILURE), (2) `HooksAssetComponent { wraps: SyntheticDataGeneratorComponent }` — outer hooks fire with the inner's DataFrame result. Callbacks are `mod:fn` refs so both shapes reference the same shared `src/<pkg>/hooks.py`. Asset-first equivalent of Dagster's job-scoped `@dg.success_hook` / `@dg.failure_hook`. |
| [Per-partition mutex — event log or Postgres advisory lock](partition_lock_asset.md) | `partition_lock_asset`, `synthetic_data_generator` | 100% offline. Demonstrates BOTH shapes AND a real held-lock conflict: (1) `@partition_lock` Python decorator on a static-partition asset, (2) `PartitionLockAssetComponent { wraps: SyntheticDataGeneratorComponent (static partitions us-east,us-west) }`. For each shape, RUN 1 acquires lock normally; then the script uses `DagsterInstance.report_runless_asset_event` to PRE-INJECT a stale "acquired" observation (simulating a concurrent holder), and RUN 2 SKIPS — compute short-circuits, inner data-gen NEVER invoked. Lock state lives in the event log; auto-expires via `ttl_seconds`; `on_conflict: wait` uses `dg.RetryRequested` (worker slot freed during backoff). |
| [Write-Audit-Publish — atomic staging → validate → promote](lifecycle_wap.md) | `lifecycle_wap` (`@lifecycle` decorator + `LifecycleWapComponent`) | 100% offline. Full Write → Audit → Publish cycle in 3 runs: (1) `py_orders_happy` — 100 clean rows, 3/3 audits pass → PUBLISHED to prod parquet; (2) `py_orders_bad` — 100 rows w/ 5 negative amounts, `amount_positive_check` FAILS → QUARANTINED (prod never written, staging preserved for triage, asset materialization fails so downstream stays blocked); (3) `yaml_orders_happy` — same shape via `LifecycleWapComponent` YAML (`compute.python` references a callable). Backends: filesystem (demoed), sql / iceberg / delta (mentioned). Audit kinds: `row_count_min`, `col_null_ratio_max`, `python` (demoed); `great_expectations` (mentioned — delegates to a GE suite). Every audit is an `AssetCheckResult` visible in Dagster's check panel. |
| [Dynamic sub-steps — Prefect's `@task` for Dagster (with cache-on-retry)](task_asset.md) | `task_asset` (`@task` / `@task_asset` decorators + `TaskAssetComponent` YAML + `child_step` context manager + `FilesystemTaskCache`) | 100% offline. Unique in DCC: FOUR shapes side-by-side. (1) `@task` alone on a plain `@dg.asset` — 3 outer + 6 nested calls emit 9 synthetic `STEP_START`/`STEP_SUCCESS` events with hierarchical step_keys in the log tab (best for agentic tool-use loops where you branch on task results). (2) `@task_asset` — same imperative body but each `@task` call is RECORDED then fanned out via `DynamicOutput`; 9 real graph nodes under `run_task[?]` with parallel execution + per-call durations (best for doc-parser / per-item LLM / scan-then-dispatch). (3) `TaskAssetComponent` YAML — `layers:` declares N compile-time-known layers referencing `mod:fn` callables; scan yields 6 items → `parse_url_process[?]` fans out into 6 mapped nodes → `terminal_reduce` summarizes (best for N-hop YAML pipelines where each hop's width is runtime-discovered). (4) `@task` + `FilesystemTaskCache` — `cache_key_fn` + `cache=` yields per-call caching auto-scoped to `root_run_id` so "Re-execute from failure" hits the cache and net-new runs never see prior runs' cache (deliberate). Setup script materializes twice; walkthrough covers the UI failure-retry recipe that exercises real cache hits. |
| [Producer/consumer schema contracts — with breaking-change detection](data_contract.md) | `data_contract` (`@data_contract` + `@requires_contract` decorators + `DataContractComponent` + `RequiresContractComponent` + `contract_from_json_schema()`) | 100% offline. FOUR features in one flowing pipeline + BOTH shapes: (1) `@data_contract` producer `orders` v1.0.0 emits contract-snapshot `AssetObservation`; (2) `@requires_contract` consumer `daily_totals` reads that observation, passes on `min_version=1.0.0`; (3) `strict_consumer` with `min_version=3.0.0` fails BEFORE compute with `dg.Failure: upstream contract version 1.0.0 < required 3.0.0`; (4) rewriting the producer to v2.0.0 (drops `status`, narrows `amount` float→int) triggers `detect_breaking_changes=True` — an ADDITIONAL observation tagged `contract_breaking_change=true` lands with a rendered markdown diff. Plus YAML shape (`DataContractComponent { compute:, contract: }` + `RequiresContractComponent { compute: | wraps: }`) and bonus `contract_from_json_schema()` helper mapping JSON Schema `type/pattern/enum/minimum/maximum` → DCC `type/regex/allowed_values/min/max`. Contract snapshot travels in every observation so downstream sensors can diff two arbitrary versions from log records alone. |
...
| [Agentic Pipeline — 5 multi-agent ops in one YAML](agentic_pipeline.md) | **`agentic_pipeline`** (one YAML declares source + N steps + sinks; each step becomes a Dagster asset) | OpenAI (default; swappable via LiteLLM) | Sibling of `ml_pipeline` / `polars_pipeline` / `warehouse_pipeline` for LLM workflows. Five ops in v1 — **`llm_call` / `route` / `debate` / `critique_loop` / `synthesize`** — chain by step id to form any multi-agent pattern. Every step's output is a first-class Dagster asset with typed numeric metadata (`cost_usd` / `latency_ms` / `tokens_total` / `n_llm_calls` / `model_fingerprint`) plus op-specific fields (arbitrator reasoning, router reasoning, all proposals, drafter/critic transcript). Per-op `kind` tag makes the catalog filterable by pipeline op. Demo partitions across 3 dates reading a different question per partition; sinks land in `out/{partition_key}/`. ~$0.001/run (gpt-4o-mini × ~10 calls). Live-validated end-to-end. |
...
| **[Local AI A/B — "should we go local" as a Dagster pipeline](local_ai_ab.md)** | **`inference_provider_ab_test` + `provider_ab_evaluator` + `inference_cost_report`** (new — three-component stack) | **Multi-provider** (OpenAI required for judge; Anthropic + local Ollama optional as candidates) | The Dagster-native answer to "should we swap in a local Ollama / Bedrock / Anthropic provider for our workload." Same prompt runs through N providers → each response an asset with cost + latency + tokens → LLM-as-judge scores all in one pass → report asset aggregates with baseline deltas + projected daily savings + recommendation. Optional `min_winner_score` asset check turns the pair into a **branch-deploy merge gate** — swap the provider in a PR, quality holds → merge; drops → merge blocked. Materialize daily → time-series in Insights. ~$0.005/run (gpt-4o-mini candidates + gpt-4o judge). |
| **[Slack Approval Gate — HITL approvals via Slack reactions](slack_approval_gate.md)** | **`slack_approval_gate`** (new) composed with existing `human_approval_gate` + `filesystem_monitor` + `asset_job` | OpenAI (for the synthetic report) + **Slack bot** (chat:write + reactions:read + reactions:write) | Closes the Airflow 2.10 HITL parity gap. Posts a report needing approval to Slack, allowlisted approvers react, sensor polls + writes the standard JSON token on quorum. Composes with the existing `HumanApprovalGateComponent` — Slack side handles posting + polling + quorum, the existing gate handles the downstream asset-check blocking. Reactions polling (not interactive buttons) so it works in Dagster+ Serverless without a public webhook. Multi-approver quorum, allowlist, timeout policies (escalate / reject / approve), escalation pings, full partition support. ~$0.001/run. |

### RAG (retrieval-augmented generation)

**Start here 👉 [RAG walkthrough index](rag.md)** — one landing page with a chooser table across all five RAG walkthroughs, ASCII asset-graph diagrams of the three shapes (retrieve-then-generate, state-tracking, planner-plus-specialists), a recommended reading order, and the RAG component palette (~40 components across sources / chunking / embeddings / vector stores / reranking / retrieval / agents / evaluators / memory / document extraction).

The five walkthroughs the landing page points at:

| Demo | Components | Requires | Highlights |
|---|---|---|---|
| [**RAG in Dagster — pick your shape**](rag.md) | — | — | Chooser + component palette; the single page to send a RAG-curious teammate to. |
| [RAG as State, not as a Pipeline](rag_state.md) | `document_corpus`, `vector_index_snapshot`, `rag_eval` | **No keys** — ChromaDB bundled ONNX embedder | Every RAG artifact as versioned state. Includes an **injected regression** — strip a term from one doc, watch the next snapshot's retrieval-quality asset check fail on its own. Retrieval-only eval, no LLM cost. Rollback = partition selector. ~3 min. |
| [Dagster + Prefect — Orchestration Meets Durable Execution](dagster_orchestrates_prefect.md) | `PrefectResourceComponent` (new), `PrefectFlowRunAssetComponent` (new), `PrefectFlowRunSensorComponent` (new) | **No keys** — local Prefect server, no cloud account | Dagster owns the graph + partitions; Prefect owns per-file work with a runtime-decided task graph (content-type-driven). Local Prefect server (`prefect server start`) + `.serve()`-deployed flow. Same YAML swaps to Prefect Cloud (`api_url` + `api_key_env_var`). Live-validated: 3 file-type variants each triggered a different task DAG inside the Prefect flow. |
| [Data Platform Showcase — Multi-Vendor Ingest → dbt → Dynamic Fan-Out](data_platform_showcase.md) | `SyntheticDataGeneratorComponent` (×3), `DuckDBTableWriterComponent` (×3), `DuckDBResourceComponent`, `dagster_dbt.DbtProjectComponent`, `DynamicFanoutAssetComponent` | **No keys** — local DuckDB, no Docker | The "typical Dagster" data platform: 3 synthetic vendors → raw DuckDB → real dbt models (3 stg + 1 fact) → dynamic fan-out over 4 customer cohorts → per-cohort CSV extracts. Uses the built-in dagster-dbt integration component + native column-level lineage. Live-validated: 500 customers + 6000 fact rows + 4 cohort extracts in one run. |
| [End-to-End RAG in Dagster](rag_complete.md) | `document_corpus`, `document_chunker`, `embeddings_generator`, `vector_store_writer`, `vector_store_query`, `reranker`, `llm_prompt_executor`, `vector_index_snapshot`, `rag_eval`, `dataframe_from_csv` | **No key** up through `reranked`; OpenAI-compatible for the LLM answer step | Two parallel paths over one 5-doc corpus: Path A = state-tracking (corpus → snapshot → eval); Path B = decomposed pipeline (chunker → embed → index → retrieve → rerank → LLM). Every component in the RAG stack as its own asset. Live-validated end-to-end. |
| [One-Component RAG, Driven by Dynamic Queries](rag_pipeline_dynamic.md) | `document_corpus`, `document_chunker`, `embeddings_generator`, `vector_store_writer`, `dataframe_from_csv`, `rag_pipeline` | **OpenAI-compatible key** for the LLM step | `rag_pipeline` (embed + retrieve + generate in one asset) partitioned by `query_id`. One materialization per query, addressable via `dg launch --partition q2`. Add/remove queries by editing a CSV, no pipeline change. Live-validated — 3 partitions each processed 1 row with grounded answers. |
| [Agentic Orchestration — Agents + Humans + Legacy Systems](agentic_orchestration.md) | `dataframe_from_csv`, `llm_prompt_executor` (×2), `human_approval_gate`, `dataframe_to_csv`, `duckdb_table_writer`, `filesystem_monitor`, `asset_job` | **OpenAI-compatible key** for agent steps | Linear pattern: 2 autonomous LLM agents (triage + specialist) → human-in-the-loop gate → post-back to a "legacy" DuckDB warehouse + ticketing sink. Partitioned per case. Sensor auto-progresses on token drop. Simpler shape — start here. |
| [Maintainer Investigation Room — AI agentic-orchestration on Dagster](maintainer_investigation_room.md) | `MCPToolPickerComponent` (GitHub MCP via stdio), `AgenticPipelineComponent` (7 steps: 4-way fan-out + synthesize + critique_loop + report), `HumanApprovalGateComponent`, `FilesystemMonitorSensorComponent`, `AssetJobComponent` | `OPENAI_API_KEY` + `GITHUB_PERSONAL_ACCESS_TOKEN` + `npx` | Fetches a real dagster-io/dagster issue via the official Anthropic GitHub MCP, runs a 4-way specialist fan-out with per-node context isolation, synthesizes triage, skeptic critiques, drafts a maintainer report, gates on a human sign-off. Every specialist output is a first-class asset key — downstream dbt / warehouse / notification assets depend on it in the same graph. Zero-to-something proven — fresh Claude Code with only `CLAUDE.md` installed composed the same shape from an NL prompt, `dagster definitions validate` PASSED. |
| [Agentic Router — One Asset per Case, Steps as Ops, Multiple Downstream Branches](agentic_router.md) | `dataframe_from_csv`, `llm_multi_path_router` (new — 6 bounded tools, 3 branches, real SQL via `duckdb_resource`), `dataframe_to_csv` (×2), `human_approval_gate`, `duckdb_table_writer`, `filesystem_monitor`, `asset_job` | **OpenAI-compatible key** | Router-loop shape from the BPMN "agentic orchestration" pitch, done Dagster-honest. Per-branch DynamicPartitionsDefinition so each branch shows only its actual case keys. Real SQL tool against DuckDB baggage_tracking table (llm_roleplay for stub tools). Per-branch structured payloads (delivery_id/address/eta for delivery_request; voucher_id/amount for voucher_issued; case_summary/urgency for escalation). Live-validated. |
| [Planner + Specialists — When One Retrieval Isn't Enough](rag_supervisor.md) | `document_corpus`, `SupervisorAgent` (with 3 tool personas) | **OpenAI-compatible key** | Planner LLM reads a multi-facet task, picks specialists from a bounded YAML-declared set, each pick fans out to its own asset (specialist LLM persona), synthesizer combines. Every runtime routing decision is a named, replayable asset. Live-validated — planner picked 3 of 3, synthesizer combined into a 4-sentence recommendation. ~$0.005/run. |
...
The resource + sink pattern for CRUD-shaped SaaS APIs (no orchestration primitive to trigger). Each vendor gets: (a) a rich `_resource` with read + write convenience methods, plus (b) a purpose-built upsert sink that mirrors a DataFrame into the vendor's data. Demos are structurally identical — DataFrame seed → resource → upsert sink → verify — so they read as a family.

| Demo | Components | Highlights |
|---|---|---|
| [Notion](notion.md) | `notion_resource`, `notion_database_upsert`, `notion_page_sync`, `inline_dataframe` | 19-method resource (search / query / create / update / append blocks / add comment / upload file), 2 sinks: multi-row DB upsert (keyed by any property, type-aware serialization) + single-page property patch (with optional markdown body replace). Handles Notion's 2025 API `databases → data_sources` split transparently. 3 back-to-back runs, 0 duplicates. |
| [GitHub](github.md) | `github_resource`, `github_issue_upsert`, `inline_dataframe` | 21-method resource (issues / PRs / workflows / releases / comments / dispatch). Sink upserts DataFrame → issues keyed by a body-embedded `<!-- dagster-key: X -->` HTML comment (survives title edits). Supports GitHub Enterprise via `api_base_url`. 3 back-to-back runs, all `0 created, 5 updated`. |
| [Jira](jira.md) | `jira_resource`, `jira_issue_upsert`, `inline_dataframe` | 16-method resource (issues / JQL search / transitions by name / comments / projects / users). Sink upserts DataFrame → issues keyed by a `dagsterkey-<value>` label so JQL filters server-side. Plain strings auto-wrapped into Atlassian Document Format. Uses the new `POST /search/jql` endpoint (Atlassian retired the old `GET /search` in 2025). 2 back-to-back runs, 0 duplicates. |
| [PagerDuty](pagerduty.md) | `pagerduty_resource`, `pagerduty_incident_upsert`, `inline_dataframe` | ~20-method resource across REST API v2 (services / incidents / users / oncalls) + Events API v2 (send_alert with dedup_key). Sink upserts DataFrame → incidents using `incident_key` for server-side dedup (PagerDuty enforces uniqueness of `incident_key` on OPEN incidents — POSTing with an existing key returns the existing one). Row status column drives forward-only transitions (triggered → acknowledged → resolved). 2 back-to-back runs, 0 duplicates. |
| [Stripe](stripe.md) | `stripe_resource`, `stripe_customer_upsert`, `inline_dataframe` | ~30-method resource covering customers / charges / payment_intents / invoices / subscriptions / products / prices / refunds / events, with native `Idempotency-Key` on every write. Sink upserts DataFrame → Stripe customers matched by email (immediately consistent `list?email=X` endpoint), fallback to `metadata.dagster_key` search when `match_by: metadata`. Idempotency key salted with Dagster run_id so within-run retries are safe but across-run deletes-then-recreates don't hit Stripe's cached "already created" response. 2 back-to-back runs, `0 created, 5 updated` on run 2. |
| [Airtable](airtable.md) | `airtable_resource`, `airtable_record_upsert`, `inline_dataframe` | ~10-method resource for bases / tables / records CRUD. Sink uses Airtable's **native server-side upsert** — `PATCH /v0/{baseId}/{table}?performUpsert[fieldsToMergeOn][]=<field>` — which handles match-and-create-or-update in one request. Auto-chunks at Airtable's 10-records-per-batch limit. `typecast: true` coerces string DataFrame values into typed fields (dates, singleSelects). Simplest upsert story of the six — no eventual consistency, no idempotency-key salt, no search-then-write dance. |
...
| [Deploying any demo to Dagster+](deploy_to_dagster_plus.md) | Meta-walkthrough | Interactive 7-step onboarder for a full dg project: `dg plus login` + CI scaffolding + env var provisioning + deploy. See also **[deploying.md](deploying.md)** (umbrella concept doc) and **[dg_deploy.md](dg_deploy.md)** (single-file wrapper CLI reference). |
