...
| [Snowflake Workspace → Dagster](snowflake_workspace.md) | `snowflake_workspace` + optional `warehouse_pipeline` (multi-step) + optional `snowflake_cortex_asset` | **One-script setup** for customers with existing Snowflake accounts. Interactive prompts → discovers every TASK / DYNAMIC TABLE / STORED PROCEDURE / STREAM / PIPE / STAGE / MATERIALIZED VIEW / EXTERNAL TABLE / ALERT in the chosen `database.schema` → user picks which entity types + name patterns to import → asks about cross-entity deps (`assets_by_name` overrides, same shape as Databricks) → optionally layers in a multi-step `warehouse_pipeline` (joins + `op: sql` + multi-sink, all compiled to one Snowflake plan per sink) and a Cortex LLM asset. Connection verified via `SELECT CURRENT_VERSION()` before scaffolding. |
