...
| [Agentic Router — One Asset per Case, Steps as Ops, Multiple Downstream Branches](agentic_router.md) | `dataframe_from_csv`, `llm_multi_path_router` (new — 6 bounded tools, 3 branches), `dataframe_to_csv` (×2), `human_approval_gate`, `duckdb_table_writer`, `filesystem_monitor`, `asset_job` | **OpenAI-compatible key** | Router-loop shape from the BPMN "agentic orchestration" pitch, done Dagster-honest: ONE graph-backed multi-asset per case (ReAct steps are ops visible in the run view, not separate assets), multi-output branches so downstream sinks show honest per-partition lineage. Baggage-loss triage: c1 → delivery_request → courier; c2 → voucher_issued + escalation → compensation + human review; c3 → escalation for review. Live-validated. |
