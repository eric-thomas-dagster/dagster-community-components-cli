...
| **[Hybrid git-runner — deploy once, iterate with `git push`](hybrid_git_runner/README.md)** | The Hybrid answer to the single-file question. A runner container (subclass of the [`script_orchestrator`](https://github.com/eric-thomas-dagster/script_scheduling_and_orchestration) `ScriptGithubComponent`) deployed ONCE to your Hybrid agent; new flows appear in Dagster+ by pushing them to a git repo the runner watches. Three deploy modes (prebuilt / build+push / --image override). Multi-deployment topology (prod/staging/dev with same runner + different env vars). Branch deployments auto-route to PR branches. |
| **[Prefect vs. Dagster+ — the single-file story](prefect_vs_dagster_single_file.md)** | Honest 3-way side-by-side (Prefect Managed / Dagster+ Serverless via wrapper / Dagster+ Hybrid via git-runner). Where each wins. Multi-deployment + branch-deployment differentiators that Prefect doesn't cleanly match. |
