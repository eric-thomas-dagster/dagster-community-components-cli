...
| [Agentic Router — Plain Dagster (no components)](agentic_router_plain.md) | Raw `@dg.asset` / `@dg.graph_asset` / `@dg.op` / `@dg.sensor` — no components | **OpenAI-compatible key** | Same asset graph shape as `agentic_router.md`, but ~250 lines of Python (vs ~100 lines of YAML). For SE demos where the audience wants to see what the pattern looks like without the framework layer. Both projects are indistinguishable in the UI. |
