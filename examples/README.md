...
| **[Event Automation (Prefect Automations shape)](event_automation.md)** | Prefect's Automations page as ONE Dagster YAML component. Triggers (`run_status` / `asset_materialized` / `schedule` / `http_poll` / `freshness_violation`) fan out to actions (`materialize` / `launch_job` / `webhook` / `slack` / `pagerduty` / `discord` / `emit_event`). Under the hood every automation becomes a real Dagster sensor — full UI, tick history, no separate mini-app. `bash setup_event_automation_demo.sh` for a live demo that fires a webhook on run failure + cron heartbeat. |
