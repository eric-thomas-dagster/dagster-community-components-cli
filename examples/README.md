...
| **[Event Automation (Prefect Automations shape)](event_automation.md)** | Prefect's Automations page as ONE Dagster YAML component. **11 trigger types** (`run_status` / `asset_materialized` / `schedule` / `http_poll` / `freshness_violation` / `run_duration` / `run_stuck` / `asset_check_failed` / `metric_threshold` / `absence` / `sqs_poll`) fan out to **17 action types** (`materialize` / `launch_job` / `cancel_run` / `retry_run` / `webhook` / `slack` / `pagerduty` / `opsgenie` / `discord` / `teams` / `mattermost` / `email` / `sns` / `sqs` / `toggle_sensor` / `toggle_schedule` / `emit_event`). Compound `all_of` + `any_of` triggers give AND/OR logic. Under the hood every automation becomes a real Dagster sensor — full UI, tick history, no separate mini-app. `bash setup_event_automation_demo.sh` for a live demo that fires a webhook on run failure + cron heartbeat. |
