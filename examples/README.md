...
| **[Single-file → Dagster+ Serverless (pex, no Docker)](single_file_serverless.md)** | The Prefect-parity story for Dagster+ Serverless. 3 files, ~30 lines of boilerplate around your actual code. Deploys as a **pex bundle** — no Docker, no image registry, no build step. Projects: [`serverless_minimal/`](./serverless_minimal/) (floor — 2 assets, zero external services), [`data_engineering_serverless/`](./data_engineering_serverless/) (real content — 5-asset HN pipeline, no API keys), [`agentic_tour_serverless/`](./agentic_tour_serverless/) (richer LLM example — needs `OPENAI_API_KEY`). Wrapper CLI at [`lib/dg_deploy_one_file.sh`](./lib/dg_deploy_one_file.sh) — `bash dg-deploy my_flow.py` auto-detects deps + `--from user/repo/path.py` GitHub fetch. **3 locations verified live at ericthomas-dagster.dagster.cloud/prod 2026-08-07**: `hello`, `cli-verify`, `data-engineering`. |
