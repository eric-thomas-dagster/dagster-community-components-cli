...
| [Maintainer Investigation Room — Prefect-parity AI agentic-orchestration](maintainer_investigation_room.md) | `MCPToolPickerComponent` (GitHub MCP via stdio), `AgenticPipelineComponent` (7 steps: 4-way fan-out + synthesize + critique_loop + report), `HumanApprovalGateComponent`, `FilesystemMonitorSensorComponent`, `AssetJobComponent` | `OPENAI_API_KEY` + `GITHUB_PERSONAL_ACCESS_TOKEN` + `npx` | Direct counter to Prefect's Aug-13 agentic-workflows pre-MVP Loom. Fetches a real dagster-io/dagster issue via the official Anthropic GitHub MCP, runs a 4-way specialist fan-out with per-node context isolation, synthesizes triage, skeptic critiques, drafts a maintainer report, gates on a human sign-off. What Prefect can't do: every specialist output is a first-class asset key, so downstream dbt / warehouse / notification assets depend on it in the same graph. Zero-to-something proven — fresh Claude Code with only `CLAUDE.md` installed composed the same shape from an NL prompt, `dagster definitions validate` PASSED. |
