...
| [Catalog Agent — meta-agent over the live 900-component registry, with schema discovery](catalog_agent.md) | **`catalog_agent`** (new — one YAML block emits N step assets + synthesis) | The apex. Per-step planner picks REAL components from the live 900-component manifest via reflection + `dg.materialize()` in-process; each step sees the ACTUAL columns/preview of every prior step's real DataFrame, so it plans against customer data with schemas unknown at pipeline-write time. Live-validated: task did NOT tell the agent what columns exist; the agent discovered `is_active`, `city`, `state`, `lifetime_value` from step 1's real output and used them in steps 2+3. Set `max_iterations: 1` for single-shot behavior. ~$0.02/run. |
