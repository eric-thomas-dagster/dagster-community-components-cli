...
| **[dbt Slim CI](dbt_slim_ci.md)** | Build only state-modified dbt models on PR deploys, keep morning schedule as full builds. Two approaches (asset_selection at CI time + code_version_changed()), full GH Action snippets for GH artifacts / dbt Cloud / S3 — all work with the stock `DbtProjectComponent`, no subclass required. | $0 |
