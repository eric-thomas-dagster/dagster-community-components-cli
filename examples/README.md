...
| [Agentic Orchestration — Agents + Humans + Legacy Systems](agentic_orchestration.md) | `dataframe_from_csv`, `llm_prompt_executor` (×2), `human_approval_gate` (new), `dataframe_to_csv`, `duckdb_table_writer`, `filesystem_monitor`, `asset_job` | **OpenAI-compatible key** for agent steps | Full pattern: 2 autonomous LLM agents (triage + specialist) → human-in-the-loop gate → post-back to a "legacy" DuckDB warehouse + ticketing sink. Partitioned per case (t1/t2/t3). Sensor auto-progresses the graph when an approval token drops. Live-validated — t1 approved+cascaded, t2 rejected+blocked, t3 sensor-triggered end-to-end. |
