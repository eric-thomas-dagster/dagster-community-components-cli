...
| [Budget asset — `@budget` decorator](budget_asset.md) | `budget_asset` (`@budget` decorator on a `@dg.asset`) | 100% offline. Per-asset $ cost tracking + rolling-window cap. Three runs of one asset each costing $0.40 via a user-supplied `cost_fn`; cumulative crosses the $1.00 cap on RUN 3 → observation tagged `budget_breach=true`. Three enforcement modes: `warn` (default), `fail` (pre-flight guard saves the run), `skip`. Cost history + breach state entirely in the event log — the shape you'd use for a FinOps dashboard. |
| [SLA asset — `@sla` decorator](sla_asset.md) | `sla_asset` (`@sla` decorator on a `@dg.asset`) | 100% offline. Wall-clock SLA + escalation-after-N-breaches. Four runs of one asset controlled by `SLEEP_SECONDS`: within-SLA → no observation; 3 back-to-back breaches → 3rd one tagged `sla_escalated=true` (sensor-actionable). Escalation counting is via `context.instance.get_event_records` — no external state. Different from `freshness_policy` (that governs source freshness); this measures compute duration. |
| [Smart retry — `@smart_retry` decorator](smart_retry.md) | `smart_retry` (`@smart_retry` decorator on a `@dg.asset`) | 100% offline. Classification-aware retry: `ConnectionError` classified transient → retries with backoff via Dagster's real `STEP_RESTARTED` machinery; `ValueError` classified permanent → `dg.Failure` immediately (no thrashing). Three runs (`success`, `flaky`, `permanent`) exercise all three paths. Optional day-1 features: LLM-classifier fallback, sliding-window rate limit, circuit breaker. Complement to Dagster's built-in `RetryPolicy` (which retries every failure the same way). |
