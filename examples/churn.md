...
`lifetime_days`), runs [`churn_prediction`](https://github.com/eric-thomas-dagster/dagster-component-templates/tree/main/assets/analytics/churn_prediction) (rule-based: inactivity
...
|---|---|---|---|
| 1 | [`csv_file_ingestion`](https://github.com/eric-thomas-dagster/dagster-component-templates/tree/main/assets/ingestion/csv_file_ingestion) | ingestion | Load 200 synthetic per-customer aggregates |
| 2 | [`churn_prediction`](https://github.com/eric-thomas-dagster/dagster-component-templates/tree/main/assets/analytics/churn_prediction) | analytics | Score each customer (inactivity threshold = 60d, lookback = 365d) and assign a risk level + recommendation |
| 3 | [`dataframe_to_csv`](https://github.com/eric-thomas-dagster/dagster-component-templates/tree/main/assets/sinks/dataframe_to_csv) | sink | Write per-customer report |
...
- **First demo using [`churn_prediction`](https://github.com/eric-thomas-dagster/dagster-component-templates/tree/main/assets/analytics/churn_prediction)** — rule-based scoring (no ML
...
- **Customer-level aggregates as input.** The component expects one
  ...
  transaction-level data, run [`summarize`](https://github.com/eric-thomas-dagster/dagster-component-templates/tree/main/assets/transforms/summarize) upstream (`group_by:
...
- **The synthetic-data heredoc pattern again.** For per-customer rollups
  ...
  [`time_series_generator`](https://github.com/eric-thomas-dagster/dagster-component-templates/tree/main/assets/analytics/time_series_generator) is for time-series, not aggregates.

## Extending

Pair with [`customer_segmentation`](https://github.com/eric-thomas-dagster/dagster-component-templates/tree/main/assets/analytics/customer_segmentation) (RFM scoring) on the same input — the
