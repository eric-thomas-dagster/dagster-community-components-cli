...
# names (e.g. delta_ingestion_orders, summarize_orders), so the defaults
# would otherwise stick around referencing non-existent upstream assets like
# `sales_data` and break `dg check defs`.
for c in synthetic_data_generator dataframe_to_delta_table delta_ingestion summarize; do
