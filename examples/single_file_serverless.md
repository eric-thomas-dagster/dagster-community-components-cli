...
If hand-writing the 2-file scaffold every time feels like too much boilerplate vs. Prefect's `prefect deploy`, use [`lib/dg_deploy.sh`](./lib/dg_deploy.sh) — it takes any single `.py` (or folder of `.py` files) with `defs = dg.Definitions(...)` at module scope and auto-generates the scaffold + deploys:
