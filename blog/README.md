...
| Date | Title |
|---|---|
| 2026-09-11 | [Augment any Dagster asset with layered components](dagster-native-decorators.md) — Dagster ships more than most teams use. The context.log stream, the event log, dg.RetryRequested, dynamic outputs, AssetCheckResult, and cheap event-log queries add up to a serious platform for cross-cutting behavior. Here are 17 decorators that wire those primitives together into retry classification, wall-clock SLAs, LLM cost caps, PII scrubbing, point-in-time snapshots, write-audit-publish, and more. |
